...
impl OfferInvocation {
    ...
    fn compile(mut self) -> Result<OfferOutput, syn::Error> {
        let chan = &self.chan;
        ...
        let case_input = CaseInput {
            scrutinee: parse_quote!(#chan),
            brace_token: self.brace_token,
            arms: self.cases,
        };

        let case_output = case_input.compile()?;
        Ok(OfferOutput {
            chan: self.chan,
            case_output,
        })
...
struct TransmitterSpec {
    name: syn::Type,
    bounds: Punctuated<TransmitterBound, Token![,]>,
}
...
struct ReceiverSpec {
    name: syn::Type,
    bounds: Punctuated<ReceiverBound, Token![,]>,
}

impl Parse for TransmitterSpec {
    fn parse(input: ParseStream) -> Result<Self, syn::Error> {
        fn parse_bound(input: ParseStream) -> Result<TransmitterBound, syn::Error> {
        ...
        if input.is_empty() {
            ...
            let bounds = if input.is_empty() {
                Punctuated::new()
            } else {
                input.parse_terminated(parse_bound)?
            };
            Ok(TransmitterSpec { name, bounds })
        }
    }
}
...
impl Parse for ReceiverSpec {
    fn parse(input: ParseStream) -> Result<Self, syn::Error> {
        fn parse_bound(input: ParseStream) -> Result<ReceiverBound, syn::Error> {
            let lookahead = input.lookahead1();
            if lookahead.peek(Token![match]) || input.is_empty() {
                Ok(ReceiverBound::Choice(input.parse()?))
            } else {
                Ok(ReceiverBound::Recv(input.parse()?))
            }
        }
...
pub fn Receiver(
    ...
    let ReceiverSpec {
        name,
        bounds: types,
    } = parse_macro_input!(params as ReceiverSpec);
    let mut item = parse_macro_input!(input as syn::Item);
    if let Some(predicates) = where_predicates_mut(&mut item) {
        predicates.push(syn::parse_quote! {
            #name: ::std::marker::Send
        ...
        for bound in types {
            match bound {
                ReceiverBound::Recv(ty) => predicates.push(syn::parse_quote! {
                    #name: #dialectic_path::backend::Receive<#ty>
                }),
                ReceiverBound::Choice(_) => predicates.push(syn::parse_quote! {
                    #name: #dialectic_path::backend::ReceiveChoice
                }),
            }
        }
        item.into_token_stream().into()
