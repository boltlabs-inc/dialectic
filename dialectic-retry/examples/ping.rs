...
async fn main() -> Result<(), Error> {
    ...
    let connector = Connector::new(connect, init, retry)
        .session::<Protocol>()
        .timeout(timeout)
        .recover_connect(backoff.backoff(ReconnectStrategy::ReconnectAfter))
        .recover_handshake(backoff.backoff(ReconnectStrategy::ReconnectAfter))
        .recover_tx(backoff.backoff(RetryStrategy::ReconnectAfter))
        .recover_rx(backoff.backoff(RetryStrategy::ReconnectAfter));
